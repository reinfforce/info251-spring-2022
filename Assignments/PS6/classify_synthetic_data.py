...
def print_classifier_fairness_stats(acc_arr, correlation_dict_arr, cov_dict_arr, s_attr_name):
    ...
    print("Protected/non-protected in positive class: %0.0f%% / %0.0f%%" % (non_prot_pos, prot_pos))
...
def compute_p_rule(x_control, class_labels):
    ...
    print("# Unprotected examples: %d" % (non_prot_all))
    print("# Protected examples: %d" % (prot_all))
    print("Unprotected in positive class: %d (%0.0f%%)" % (non_prot_pos, non_prot_pos * 100.0 / non_prot_all))
...
def train_test_classifier(X_train, y_train, x_sensitive_train, X_test, y_test, x_sensitive_test, sensitive_attrs,
    ...
    return theta, p_rules, test_score, np.array(distances_from_decision_boundary)
